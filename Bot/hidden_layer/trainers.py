import logging
from input_layer import InputObject
from output_layer import OutputObject
from data_layer.corpus import Corpus
...
class Trainer(object):
    ...
    def get_or_create(self, statement_text):
        ...
        if not statement:
            statement = InputObject(statement_text)
        ...
        for text in conversation:
            ...
            if statement_history:
                statement.add_response(
                    OutputObject(statement_history[-1].text)
                )
...
class TwitterTrainer(Trainer):
    ...
    def get_statements(self):
        ...
        for tweet in tweets:
            statement = Input(tweet.text)

            if tweet.in_reply_to_status_id:
                try:
                    status = self.api.GetStatus(tweet.in_reply_to_status_id)
                    statement.add_response(Output(status.text))
            ...
            with open(file, 'r') as tsv:
                ...
                for row in reader:
                    if len(row) > 0:
                        ...
                        if statement_history:
                            statement.add_response(
                                Output(statement_history[-1].text)
                            )
